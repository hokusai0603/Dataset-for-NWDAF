...
generate_cat2.py
================
Traffic generator for Category 2 (Bursty & Interactive) using only
the 'search' action (longest total session duration in Cat2).
...
Usage:
    python generate_cat2.py --runs 15 --min-duration 150 --max-duration 450
"""
...
# ── Config ────────────────────────────────────────────────────────────
# Fixed action for Cat2: search (longest total session duration)
FIXED_ACTION = "search"


def parse_args():
    p = argparse.ArgumentParser(description="Cat2 Traffic Generator (search only)")
    ...
    p.add_argument("--output-dir", type=str, default="cat2", help="Output directory")
    return p.parse_args()
...
def main():
    ...
    print("=" * 60)
    print("Cat2 Traffic Generator (search only)")
    ...
    for run_id in range(1, args.runs + 1):
        ...
        correlation_id = f"cat2_{int(base_time.timestamp())}_run{run_id:03d}"

...
generate_cat1.py
================
Traffic generator for Category 1 (Real-time & Streaming) using only
the 'video-streaming' action (longest total session duration in Cat1).
...
Usage:
    python generate_cat1.py --runs 15 --min-duration 150 --max-duration 450
"""
...
# ── Config ────────────────────────────────────────────────────────────
# Fixed action for Cat1: video-streaming (longest total session duration)
FIXED_ACTION = "video-streaming"


def parse_args():
    p = argparse.ArgumentParser(description="Cat1 Traffic Generator (video-streaming only)")
    ...
    p.add_argument("--output-dir", type=str, default="cat1", help="Output directory")
    return p.parse_args()
...
def main():
    ...
    print("=" * 60)
    print("Cat1 Traffic Generator (video-streaming only)")
    ...
    for run_id in range(1, args.runs + 1):
        ...
        correlation_id = f"cat1_{int(base_time.timestamp())}_run{run_id:03d}"
